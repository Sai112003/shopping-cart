...
function addToCart(product_id,product_name,imgSrc,price)
{
	...
	localStorage.setItem("finalHTML",flexOutput.innerHTML)
	 ...
	 let myPrices=localStorage.getItem("prices").split(",")
	 myPrices.push(price)
	 localStorage.setItem("prices",myPrices)

	 let myImgs=localStorage.getItem("imgSrcs").split(",")
	 myImgs.push(imgSrc)
	 localStorage.setItem("imgSrcs",myImgs)
	}
}
else
{
...
let uname=prompt("Introduce Yourself")
	...
	returnDiv=`<div id="flexContainer">
			   ...
			   <label>Price:</label><span style="margin-left:90px;">₹${price}</span><br>
			   <hr><button class="badge badge-pill badge-danger" style="padding:10px;" onclick="removeItem(${product_id})"><i class="bi bi-trash3-fill"></i></i></button></div><div id="buttons" style="background-color:none;background-image:none;border:none;"><button class="btn btn-danger" onclick="clearCart()" style="float:left;margin:auto auto auto 10px;">Clear Cart</button>
...
let mytable=`<div id="billTable"><table border="1">
    <tr>
        <th>Product</th>
        <th>Quantity</th>
        <th>Price</th>
...
for(let k=0;k<myImgArr.length;k++)
{
	let tot=Number(qty[k])*Number(prices[k])
	grandTotal+=tot
	mytable+=`<tr><td><img src="${myImgArr[k]}" style="width:100px;height:100px;object-fit:cover;"></td><td>${qty[k]}</td><td>${prices[k]}</td><td>${tot}</td></tr>`
}
mytable+=`<tr><td colspan="3">Grand Total</td><td>${grandTotal}</td></tr></table></div><div id="buttons2"><button id="backTocart" class="btn btn-danger" onclick="goBack()">Back</button><button class="float-right btn btn-success" id="pay" onclick="proceedpayment(${grandTotal})">Proceed To Pay </button></div>
             <div id="bankInfo" class="bg-info"></div> `



document.getElementById("finalBill").style.display="block"
document.getElementById("finalBill").innerHTML=mytable
...
{
let cardDiv=`<img src="../IMAGES/visa.png"><img src="../IMAGES/rupay.png"><img src="../IMAGES/master card.jpg"><img src="../IMAGES/paytm.png"> `
cardDiv+=`<H4 style="margin=10px auto auto auto">Amount payable:₹${total}</H4>`

cardDiv+=`<div style="width:510px;margin:auto;"><div style="float:left;margin:10px auto auto auto;"><lable>Card Number:</lable><input type="text" style="width:50px;text-align:center;" maxlength="4">-<input type="text" style="width:50px;text-align:center;" maxlength="4">-<input type="text" style="width:50px;text-align:center;" maxlength="4">-<input type="text" style="width:50px;text-align:center;" maxlength="4"></div>`
cardDiv+=`<div style="margin:10px 10px auto auto;float:right;">cvv/cvv2:<input type="password" style="appearance:none;padding-left:15px;width:70px;background-image:url('../IMAGES/lock.svg');background-size:15px auto;background-position:1px 5px;background-repeat:no-repeat;" maxlength="3"></div></div>`
cardDiv+=`<div style="margin:10px auto auto 27px;clear:left;"><label>Valid thru:</label><input type="text" style="width:40px;text-align:center;margin-top:10px;" maxlength="2">/<input type="text" style="width:40px;tet-align:center;margin-top:10px;" maxlength="2"></div><hr style="margin:0px;padding:0px;">`
